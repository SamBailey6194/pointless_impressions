(()=>{function w(t){let e=document.getElementById("artwork-list");if(!e)return;let s=typeof getCloudinaryUrl<"u"&&typeof ARTWORK_DETAIL_BASE_PATH<"u",a=typeof ARTWORK_LIST_URL<"u"?ARTWORK_LIST_URL:"/artworks/",i="";t.length===0?i=`
            <div class="hero min-h-[400px] col-span-full">
                <div class="hero-content text-center mx-auto">
                    <div class="max-w-md">
                        ...
                        <a href="${a}" class="clear-filters btn btn-primary">
                ...
                </div>`;i+=`<div class="artwork-card card shadow-xl hover:shadow-2xl transition-shadow" data-sku="${n.sku}">${r}${L}</div>`}),e.innerHTML=i}function k(t){return t.filter(e=>e.is_available||e.is_in_stock)}function h(t){return[...t].sort((e,s)=>e.price-s.price)}function S(t){return[...t].sort((e,s)=>s.price-e.price)}function x(t){return[...t].sort((e,s)=>e.name.localeCompare(s.name))}function R(t){return[...t].sort((e,s)=>{let a=e.artist?.username||e.name,i=s.artist?.username||s.name;return a.localeCompare(i)})}var u=[],l="price",d="asc",v=!1;function b(){let t=[...u];switch(l){case"price":t=d==="desc"?S(t):h(t);break;case"name":t=x(t);break;case"artist":t=R(t);break;default:t=h(t)}w(t)}function f(){document.querySelectorAll('.sort-control[data-type="sort"]').forEach(t=>{let e=t.getAttribute("data-sort-key"),s=t.getAttribute("data-sort-direction");String(e)===String(l)&&String(s)===String(d)?t.classList.add("btn-active"):t.classList.remove("btn-active")})}function T(t){let e=t.target.closest(".sort-control");if(!e||e.dataset.type!=="sort"||!v||u.length===0)return;t.preventDefault();let s=e.dataset.sortKey,a=e.dataset.sortDirection;l=s,d=a,f(),b();let i=new URL(window.location);i.searchParams.set("sort",s),i.searchParams.set("direction",a),window.history.pushState({},"",i)}function E(){let t=new URLSearchParams(window.location.search);l=t.get("sort")||"price",d=t.get("direction")||"asc",f()}function U(){if(E(),f(),typeof window.ARTWORKS_JSON_DATA<"u"&&window.ARTWORKS_JSON_DATA.length>0){u=window.ARTWORKS_JSON_DATA,v=!0;let e=new URLSearchParams(window.location.search);(e.has("sort")||e.has("direction"))&&b()}let t=document.getElementById("controls");t&&t.addEventListener("click",T)}function $(){let t=document.getElementById("min_price"),e=document.getElementById("max_price"),s=document.getElementById("apply-filters"),a=s?.closest("form");if(!t||!e||!s||!a)return;function i(){let n=parseFloat(t.value)||0,r=parseFloat(e.value)||0;t.value&&e.value&&n>r?(s.disabled=!0,s.title="Min price cannot be greater than max price",s.classList.add("btn-disabled")):(s.disabled=!1,s.title="",s.classList.remove("btn-disabled"))}t.addEventListener("change",i),t.addEventListener("input",i),e.addEventListener("change",i),e.addEventListener("input",i),a.addEventListener("submit",function(n){let r=parseFloat(t.value)||0,o=parseFloat(e.value)||0;t.value&&e.value&&r>o&&(n.preventDefault(),alert("Min price cannot be greater than max price"))}),i()}document.addEventListener("DOMContentLoaded",function(){U(),$()});})();
